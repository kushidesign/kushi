...
(defn colorway-args [s]
  (let [base
   ...
   [{"[data-ks-surface= \"transparent\"]"                   base

     "[data-ks-surface= \"transparent\"][data-ks-inert]" base-inert

     "[data-ks-surface= \"minimal\"]"                       (merge base
     ...
     "[data-ks-surface= \"minimal\"][data-ks-inert]"     base-inert

     "[data-ks-surface= \"faint\"]"                         {:bgc             (keyword (str "$background-color-" s "-soft"))
     ...
     "[data-ks-surface= \"faint\"][data-ks-inert]"       {:bgc             (keyword (str "$background-color-" s "-soft"))
                                                                :hover:bgc       (keyword (str "$background-color-" s "-soft"))
                                                                :active:bgc      (keyword (str "$background-color-" s "-soft"))
                                                                :dark:bgc        (keyword (str "$background-color-" s "-soft-dark-mode"))
                                                                :dark:hover:bgc  (keyword (str "$background-color-" s "-soft-dark-mode"))
     ...
     "[data-ks-surface= \"soft\"]"                          soft

     "[data-ks-surface= \"soft\"][data-ks-inert]"        soft-inert

     "[data-ks-surface= \"soft-classic\"]"                  (merge soft
                                                                      {:--classic-trim-color      (keyword (str "$" s "-150"))
                                                                       :--classic-trim-color-dark (keyword (str "$" s "-800"))})

     "[data-ks-surface= \"solid\"]"                         {:bgc        (keyword (str "$background-color-" s "-hard"))
                                                                :hover:bgc  (keyword (str "$background-color-" s "-hard-2"))
                                                                :active:bgc (keyword (str "$background-color-" s "-hard-3"))}

     "[data-ks-surface= \"solid\"][data-ks-inert]"       {:bgc        (keyword (str "$background-color-" s "-hard"))
     ...
     "[data-ks-surface= \"solid-classic\"][inert]"          {:bgc                  (keyword (str "$background-color-" s "-hard"))
                                                                :hover:bgc            (keyword (str "$background-color-" s "-hard"))
                                                                :active:bgc           (keyword (str "$background-color-" s "-hard"))}
     ...
     "dark:[data-ks-surface= \"solid-classic\"][inert]"     {:bgc        (keyword (str "$background-color-" s "-hard-dark-mode"))
     ...
     "dark:[data-ks-surface= \"solid\"][inert]"             {:bgc        (keyword (str "$background-color-" s "-hard-dark-mode"))
                                                                :hover:bgc  (keyword (str "$background-color-" s "-hard-dark-mode"))
                                                                :active:bgc (keyword (str "$background-color-" s "-hard-dark-mode"))}
     }]))
