(ns kushi.css.build.analyze
  (:require
   ...
   [kushi.core :refer [extract-css-props]]
...
(defn css-string-call-data
   ...
   *css]
  (let [[layer x] (if (= 2 (count args))
                    args
                    ["@layer user-shared-styles" (first args)])
        css         (cond (string? x) x
                          (and (list? x) (-> x first keyword) (-> x second map?))
                          (let [[f opts] x]
                            (cond (= f :data-ks-surface-str)
                                  (surfaces/data-ks-surface-str opts))))
