...
(defn shadow-and-stroke-attrs*
  ...
  (let [only-simple-stroke?      (and (contains? stroke-presets-key-set 
                                                 stroke)
                                      (not shadow))
        only-simple-shadow? (and (contains? shadow-presets-key-set
                                                 shadow)
                                      (not stroke))]
    (!? :pp (cond 
              only-simple-stroke?
...
(defn shadow-and-stroke-attrs [props]
  (or (:kushi.ui.core/pc props)
      (shadow-and-stroke-attrs* props)))
