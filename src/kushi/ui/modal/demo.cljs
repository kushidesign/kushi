...
(def examples
  (let [row-attrs (sx :&_.kushi-button:fs--$small)]
    [
     #_(let [code (sx-call (let [id "my-modal-basic"]
                           [:div
                            [button
     ...
     #_{:desc      "With modal title, description, form with fields, and close button"
      :component button
      :reqs      '[[kushi.ui.button.core :refer [button]]]
      :row-attrs row-attrs
