...
(defn color-token-scale
  ...
  (into [[(keyword (str "--" (or alias color-name) "-hue-oklch" )) hue]]
        (for [[lvl chroma lightness] scale]
...
(defn oklch-colors-expanded* []
  (!? :pp
     (->> (for [[color-name {:keys [scale] :as m}]
                ...
                :let                                  
                [fifty    (first scale)
                 others   (->> scale rest (partition 2))
                 expanded (->> others
                               (map (fn [[[lvl-a chroma-a lightness-a]
                                          [lvl-b chroma-b lightness-b]]]
                                      [[lvl-a chroma-a lightness-a]
                                       [(+ 50 lvl-a)
                                        (round2 2 (/ (+ chroma-a chroma-b) 2))
                                        (round2 2 (/ (+ lightness-a lightness-b)
                                                     2))]
                                       [lvl-b chroma-b lightness-b]]))
                               (concat [[fifty]])
                               (apply concat)
